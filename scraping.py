...
def mars_news(browser):
    ...
    # Convert the browser htmlm to a soup object and then quit the browser
    html = browser.html
...
def hemisphere_info(browser):
    ...
    hem_elem = browser.find_by_css("h3")
    try:
        for i in range(4):
            hem_elem[i].click()
            hem_soup = soup(browser.html, 'html.parser')
            hem_url = hem_soup.ul.find('a')['href']
            hem_name = browser.find_by_css("h2").text
            hemisphere_image_urls.append({ 'img_url' : url + hem_url, "title": hem_name} )
            browser.back()
    except BaseException:
        return None
